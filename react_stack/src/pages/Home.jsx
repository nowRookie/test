...
export default class Home extends React.Component {
  ...
  grid() {
    ...
    let height = (window.innerHeight || document.documentElement.clientHeight || document.body.clientHeight) - 50
    if (1000 < width < 1600) {
      x = 8
    }
    if (width < 1000) {
      ...
      <div className="home">
        ...
        <div className="cf desktop">
          {
            _.map(this.state.units, (item, i) => {
              return (
                <div key={item.id} className="tc item" onDoubleClick={this.show.bind(this, item)}
                  onMouseDown={this.move.bind(this)}
                  style={{
                    width: this.state.unitWidth, height: this.state.unitHeight,
                    // transform: `translate(${item.translateX}px, ${item.translateY}px)`
                    top: item.translateY, left: item.translateX
                  }}>
                  <div className="g">
                    <p className="p1" style={{ height: this.state.unitHeight - 14 }}>
                      <img src={item.imgUrl} style={{ width: "80%", height: this.state.unitHeight - 24 }} />
                    </p>
        ...
        <div className="oh modal">
          <div className="bb header">
            ...
            <a className="fr close" onClick={this.close}>&#xe685;</a>
          </div>
          <div className="content">
            {
              this.state.modalContent.length ?
                _.map(this.state.modalContent, (item, i) => {
                  return <a className="fl a" key={item.id} onClick={this.linkTo.bind(this, item)}>{item.title}</a>
                }) :
